...
public class DbSearcher {
     ...
     */
    public DbSearcher(DbConfig dbConfig, String dbFile, QueryType queryType) throws IOException {
        ...
        switch (queryType) {
            ...
            case BINARY:
                raf.seek(0L);
                byte[] superBytes = new byte[DbMaker.SUPER_PART_LENGTH];
     ...
     * @param bytes
     */
    public DbSearcher(DbConfig dbConfig, byte[] bytes) {
        this.dbConfig = dbConfig;
        this.dbBinStr = bytes;
        this.dbBinStr = bytes;
    ...
    private void initMemoryOrBinaryModeParam(byte[] bytes, long fileSize) {
        ...
        long fileSizeInFile = Util.getIntLong(bytes, DbMaker.FILE_SIZE_PTR);
        if (fileSizeInFile != fileSize) {
            throw new RuntimeException("db file size error");
        }
        firstIndexPtr = Util.getIntLong(bytes, DbMaker.FIRST_INDEX_PTR);
        lastIndexPtr = Util.getIntLong(bytes, DbMaker.END_INDEX_PTR);
    ...
    private void initBtreeModeParam(RandomAccessFile raf) throws IOException {
        ...
        ipBytesLength = dbType == DbType.IPV4 ? 4 : 16;
        long fileSizeInFile = Util.getIntLong(superBytes, DbMaker.FILE_SIZE_PTR);
        if (fileSizeInFile != raf.length()) {
            throw new RuntimeException("db file size error");
        }
        ...
        for (int i = 0; i < b.length; i += indexLength) {
            dataPtr = Util.getIntLong(b, i + 16);
            if (dataPtr == 0) {
                break;
            }
     ...
     * @throws IOException
     */
    private DataBlock memorySearch(byte[] ip) throws IOException {
        ...
        while (l <= h) {
            ...
            if (compareBytes(ip, sip, ipBytesLength) < 0) {
                ...
                if (compareBytes(ip, eip, ipBytesLength) > 0) {
                    ...
                    dataWrapperPtr = Util.getIntLong(dbBinStr, p + 32);
                    break;
                }
            }
        }
        ...
        String region = new String(dbBinStr, dataPtr, dataLen, "UTF-8");
        ...
        while (l <= h) {
            ...
            if (compareBytes(ip, sip, ipBytesLength) < 0) {
                ...
                if (compareBytes(ip, eip, ipBytesLength) > 0) {
                    ...
                    dataWrapperPtr = Util.getIntLong(iBuffer, p + 32);
                    break;
                }
            }
        }
        ...
        while (l <= h) {
            ...
            if (compareBytes(ip, sip, ipBytesLength) < 0) {
                ...
                if (compareBytes(ip, eip, ipBytesLength) > 0) {
                    ...
                    dataWrapperPtr = Util.getIntLong(buffer, 32);
                    break;
                }
            }
        }
    ...
    private DataBlock getByIndexPtr(long ptr) throws IOException {
        ...
        raf.readFully(buffer, 0, buffer.length);
        long extra = Util.getIntLong(buffer, 32);
